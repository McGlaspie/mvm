...
local kAlienAlertSound = PrecacheAsset("sound/NS2.fev/marine/voiceovers/commander/command_station_under_attack")	//TODO Change somehow
...
local function GetTextureCoords(teamIndex, techId)
	...
	//TODO Update to show different Tech-Focuses, per CS
...
function GUIInsight_TechPoints:UpdateTechPoint(techPoint, techPointRecord, currentPointIndex)
    ...
    if teamChanged then
        ...
        storedValues.PowerTeam1 = -1
        storedValues.PowerTeam2 = -1
    ...
    if team == kTeam1Index then
        ...
        if powerFraction ~= storedValues.PowerTeam1 then
        
            if not teamChanged and powerFraction < storedValues.PowerTeam1 then
                storedValues.DamageFlashTime = currentTime + kAttackFlashTime
                if currentTime - lastAlertTime > kTimeBetweenAlerts then
                    lastAlertTime = currentTime
                    playAlertSound(team)
            ...
            storedValues.PowerTeam1 = powerFraction
            
        end
        
    elseif team == kTeam2Index then
		
        if powerFraction ~= storedValues.PowerTeam2 then
        
            if not teamChanged and powerFraction < storedValues.PowerTeam2 then
                storedValues.DamageFlashTime = currentTime + kAttackFlashTime
                if currentTime - lastAlertTime > kTimeBetweenAlerts then
                    lastAlertTime = currentTime
                    playAlertSound(team)
            ...
            storedValues.PowerTeam2 = powerFraction
