...
function GUIInsight_Overhead:Uninitialize()
    ...
    if self.playerHealthbars then
        GetGUIManager():DestroyGUIScriptSingle("mvm/Hud/GUIInsight_PlayerHealthbars")
        self.playerHealthbars = nil
    end
    if self.otherHealthbars then
        GetGUIManager():DestroyGUIScriptSingle("mvm/Hud/GUIInsight_OtherHealthbars")
...
function GUIInsight_Overhead:Update(deltaTime)
    ...
    if not PlayerUI_IsCameraAnimated() then
        ...
        if self.playerHealthbars == nil then
            self.playerHealthbars = GetGUIManager():CreateGUIScriptSingle("mvm/Hud/GUIInsight_PlayerHealthbars")
        end
        if self.otherHealthbars == nil then
            self.otherHealthbars = GetGUIManager():CreateGUIScriptSingle("mvm/Hud/GUIInsight_OtherHealthbars")
        end
    ...
    if entity ~= nil and HasMixin(entity, "Live") and entity:GetIsAlive() then
        ...
        if HasMixin(entity, "Construct") then
            if not entity:GetIsBuilt() then
                ...
                local constructionStr = string.format(" (%d%% %s)", math.ceil(entity:GetBuiltFraction()*100), "Built")
