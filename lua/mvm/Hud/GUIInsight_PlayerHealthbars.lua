...
function GUIInsight_PlayerHealthbars:UpdatePlayers(deltaTime)
    ...
    for index, player in ientitylist(players) do
        ...
        if relevant then
            ...
            local textColor = Color(kNameTagFontColors[player:GetTeamNumber()])
            local healthColor = kHealthColors[player:GetTeamNumber()]
            local armorColor = kArmorColors[player:GetTeamNumber()]
            local isPoisoned = player.poisoned	//may cause errors
            local isParasited = player.parasited	//may cause errors
