...
local kPickupsVisibleRange = 15		//Move to BalanceMisc?
...
function GUIPickups:GetFreePickupGraphic()
    ...
    newPickupGraphic:SetShader("shaders/GUI_TeamThemed.surface_shader")
...
function GUIPickups:UpdateTeamColors( uiItem )
	
	local player = Client.GetLocalPlayer()	//FIXME Remove this condition check on every update
	...
	uiItem:SetFloatParameter( "teamBaseColorR", ui_baseColor.r )
    uiItem:SetFloatParameter( "teamBaseColorG", ui_baseColor.g )
    uiItem:SetFloatParameter( "teamBaseColorB", ui_baseColor.b )
    ...
    if localPlayer then
        ...
        for i, pickup in ipairs(nearbyPickups) do
            ...
            if dotProduct > 0 then
                ...
                self:UpdateTeamColors( freePickupGraphic )
