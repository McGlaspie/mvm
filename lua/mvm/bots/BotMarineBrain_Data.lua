// Just a staight copy of vanilla file. Changes were needed for MvM
// support are added at the appropriate points. I didn't want to use
// the modding framework for bots (too many potential issues).
//
//
//-----------------------------------------------------------------------------
...
//  Phase gates
//----------------------------------------
local function FindNearestPhaseGate(fromPos, favoredGateId, teamNumber )

    local gates = GetEntitiesForTeam( "PhaseGate", teamNumber )
...
//----------------------------------------
local function GetPhaseDistanceForMarine( marinePos, to, lastNearestGateId, teamNumber )
	
    local p0Dist, p0 = FindNearestPhaseGate( marinePos, lastNearestGateId, teamNumber )
    local p1Dist, p1 = FindNearestPhaseGate( to, nil, teamNumber )
...
local function PerformMove( marinePos, targetPos, bot, brain, move )
    ...
    local dist, gate = GetPhaseDistanceForMarine( marinePos, targetPos, brain.lastGateId, bot:GetPlayer():GetTeamNumber() )
...
*/
    function(bot, brain)
        ...
        local weaponTechs = { 
			[kTechId.ShotgunTech] = kTechId.Shotgun, 
			[kTechId.GrenadeLauncherTech] = kTechId.GrenadeLauncher, 
			[kTechId.FlamethrowerTech] = kTechId.Flamethrower 
		}
...
//??? Could this be limited to a location?
//If so, then creating a defend order should limit to the location of said order
...
local function GetAttackUrgency( bot, mem )
	...
	if mem.bteam == bot:GetPlayer():GetTeamNumber() or mem.bteam == kTeamReadyRoom then
		return nil
	end
	...
	*/
		[kMinimapBlipType.SentryBattery] = numOthers >= 2           and 0.2 or 0.95, // kind of a special case
        [kMinimapBlipType.CommandStation] = numOthers >= 6           and 0.5 or 0.9,
        [kMinimapBlipType.Extractor] = numOthers >= 2      and 0.4 or 0.8,
        [kMinimapBlipType.InfantryPortal] = numOthers >= 1            and 0.2 or 0.5,
        [kMinimapBlipType.Observatory] = numOthers >= 2          and 0.2 or 0.5,
        [kMinimapBlipType.PhaseGate] = numOthers >= 2          and 0.2 or 0.5,
        [kMinimapBlipType.PrototypeLab] = numOthers >= 2          and 0.2 or 0.5,
        [kMinimapBlipType.Armory] = numOthers >= 2           and 0.2 or 0.5,
        [kMinimapBlipType.AdvancedArmory] = numOthers >= 2           and 0.2 or 0.5,
        [kMinimapBlipType.MAC] = numOthers >= 1 and 0.2 or 0.5,
        [kMinimapBlipType.RoboticsFactory] = numOthers >= 1 and 0.2 or 0.5
    }
    ...
    function EvalActiveUrgenciesTable(numOthers)
        ...
        local activeUrgencies =
        {
            ...
            */
            ...
            [kMinimapBlipType.ARC] = numOthers >= 1 and 0.1 or 1.0,
            [kMinimapBlipType.Sentry] = numOthers >= 2  and 0.1 or 2.0,
            [kMinimapBlipType.JetpackMarine] = numOthers >= 2   and 0.1 or 3.0,
            [kMinimapBlipType.Marine] = numOthers >= 2  and 0.1 or 4.0,
            [kMinimapBlipType.Exo] = numOthers >= 2   and 0.1 or 5.0
...
function CreateMarineBrainSenses()
    ...
    s:Add("nearestArmory", function(db)
            ...
            local marine = db.bot:GetPlayer()
            local marineTeam = marine:GetTeamNumber()
            local armories = GetEntitiesForTeam( "Armory", marineTeam )

            local dist, armory = GetMinTableEntry( armories,
                function(armory)
                    assert( armory ~= nil )
                    if armory:GetIsBuilt() and armory:GetIsPowered() then
                        local dist,_ = GetPhaseDistanceForMarine( marine:GetOrigin(), armory:GetOrigin(), db.bot.brain.lastGateId, marineTeam )
	...
	*/
    s:Add("nearestWeldable", function(db)
            ...
            local marine = db.bot:GetPlayer()
            local targets = GetEntitiesWithMixinForTeamWithinRange( "Weldable", marine:GetTeamNumber(), marine:GetOrigin(), 10.0 )
