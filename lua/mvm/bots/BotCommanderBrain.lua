...
//  
//----------------------------------------
class 'BotCommanderBrain' (BotPlayerBrain)

function BotCommanderBrain:GetShouldDebug(bot)
    //return true
    return gDebug
end
...
//  The result is a hash table with techId as keys and an array of units as values. 
//  These are the units that you can perform the tech on (it may just be com)
//----------------------------------------
function BotCommanderBrain:GetDoableTechIds(com)

	PROFILE("BotCommanderBrain:Update")
...
//----------------------------------------
function BotCommanderBrain:ExecuteTechId( commander, techId, position, hostEntity )
    ...
    local success, keepGoing = 
		commander:ProcessTechTreeActionForEntity(
            techNode,
            position,
            Vector(0,1,0),  // normal
...
//  
//----------------------------------------
function BotCommanderBrain:Update( bot, move )
    ...
    bot:GetMotion():SetDesiredMoveDirection( 
...
Event.Hook("Console_bot_com",
    function()
        gDebug = not gDebug
        Print("BotCommanderBrain debug = %s", ToString(gDebug))
    end)
