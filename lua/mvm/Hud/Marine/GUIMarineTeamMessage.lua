...
function GUIMarineTeamMessage:UpdateTeamColors()

	local playerTeam = PlayerUI_GetTeamNumber()
	local ui_bgColor = ConditionalValue(
		playerTeam == kTeam1Index,
		kGUI_Team1_BaseColor,
		kGUI_Team2_BaseColor
	)
	local ui_fontColor = ConditionalValue(
		playerTeam == kTeam1Index,
		kMarineFontColor,
		kAlienFontColor
	)
	
	self.background:SetColor( ui_bgColor )
	self.messageText:SetColor( ui_fontColor )
