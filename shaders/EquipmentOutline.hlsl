...
float4 FinalCompositePS(PS_INPUT input) : COLOR0
{
	...
	float4 teamOneColor = float4( 0, 3, 5, 1 );
	float4 teamTwoColor = float4( 5, 2, 0, 1 );
	
	float4 glowColor;
	if( team == 1 ) {
		glowColor = teamOneColor;
	} else if( team == 2 ) {
		glowColor = teamTwoColor;
	} else if( team == 0 ) {
		glowColor = float4( 1, 1, 1, 1 );
	}
	
    if (glow.r > 0) {
		
        float opacity = 1 - tex2D( inputTexture2, input.texCoord).a;
        result += glow * glowColor * opacity * 0.25;  
