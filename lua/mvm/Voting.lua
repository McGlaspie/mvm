...
Shared.RegisterNetworkMessage("MvM_SendVote", { voteId = "integer", choice = "boolean" })
kVoteState = enum( { 'InProgress', 'Passed', 'Failed' } )

Shared.RegisterNetworkMessage("MvM_VoteResults", {
	voteId = "integer", 
	yesVotes = "integer (0 to 255)", 
	noVotes = "integer (0 to 255)", 
	requiredVotes = "integer (0 to 255)", 
	state = "enum kVoteState" 
})

Shared.RegisterNetworkMessage( "MvM_VoteComplete", { voteId = "integer" })

kVoteCannotStartReason = enum({
	'VoteAllowedToStart', 'VoteInProgress', 'Waiting', 'Spam', 'DisabledByAdmin'
})

Shared.RegisterNetworkMessage("MvM_VoteCannotStart", { reason = "enum kVoteCannotStartReason" })
...
if Server then
    ...
    local function StartVote(voteName, client, data)
        ...
        if voteCanStart == kVoteCannotStartReason.VoteAllowedToStart then
            ...
            Server.SendNetworkMessage(client, "MvM_VoteCannotStart", { reason = voteCanStart }, true)
    ...
    Server.HookNetworkMessage("MvM_SendVote", OnSendVote)
    
    local function GetVotePassed(yesVotes, noVotes)
        return yesVotes > (Server.GetNumPlayers() / 2)
    end
    
    local function MvM_OnUpdateVoting(dt)
        ...
        if activeVoteName then
            ...
            if Shared.GetTime() - lastTimeVoteResultsSent > 1 then
                ...
                Server.SendNetworkMessage("MvM_VoteResults", { voteId = activeVoteId, yesVotes = yes, noVotes = no, state = voteState, requiredVotes = required }, true)
            ...
            if Shared.GetTime() - activeVoteStartedAtTime >= delay then
                ...
                Server.SendNetworkMessage("MvM_VoteComplete", { voteId = activeVoteId }, true)
    ...
    Event.Hook("UpdateServer", MvM_OnUpdateVoting)
...
if Client then
    ...
    function SendVoteChoice(votedYes)
        ...
        if currentVoteId > 0 then
            ...
            Client.SendNetworkMessage("MvM_SendVote", { voteId = currentVoteId, choice = votedYes }, true)
    ...
    Client.HookNetworkMessage("MvM_VoteResults", OnVoteResults)
    ...
    Client.HookNetworkMessage("MvM_VoteComplete", OnVoteComplete)
    ...
    Client.HookNetworkMessage("MvM_VoteCannotStart", OnVoteCannotStart)
