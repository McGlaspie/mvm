...
RegisterVoteType("MvM_VoteKickPlayer", { kick_client = "integer" })
...
if Client then
    ...
    local function SetupKickPlayerVote(voteMenu)
        ...
        local function StartKickPlayerVote(data)
            AttemptToStartVote("MvM_VoteKickPlayer", { kick_client = data.kick_client })
        end
        ...
        AddVoteStartListener("MvM_VoteKickPlayer", GetVoteKickPlayerQuery)
...
if Server then
    ...
    SetVoteSuccessfulCallback("MvM_VoteKickPlayer", kExecuteVoteDelay, OnKickPlayerVoteSuccessful)
